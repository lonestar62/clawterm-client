...
function handleBabySession(ws, applid, route) {
  ...
  function termSend(text) {
    if (ws.readyState === ws.OPEN) ws.send(JSON.stringify({ _ctrl: 'data', text: text }));
  }

  gws = new WebSocket('ws://' + route.host + ':' + route.port + '/');
  ...
  gws.on('message', (raw) => {
    ...
    if (msg.type === 'res' && msg.payload && msg.payload.type === 'hello-ok') {
      ...
      termSend('\r\n\x1b[32m[VT]\x1b[0m \u2713 CF_ACCEPT \u2014 ' + applid + ' live\r\n\x1b[90m    All layers green. Type and press Enter.\x1b[0m\r\n\r\n\x1b[32m$\x1b[0m ');
  ...
  // Browser -> agent: decode CNA frames, buffer until Enter
  ws.on('message', (data) => {
    // Check for JSON ctrl
    try {
    ...
    // Decode CNA binary frame or plain text
